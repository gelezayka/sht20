...
volatile unsigned char xdata PETITMODBUS_SLAVE_ADDRESS         =1;
...
PETIT_RXTX_DATA xdata    Petit_Tx_Data;
unsigned int    xdata    Petit_Tx_Current              = 0;
unsigned int    xdata    Petit_Tx_CRC16                = 0xFFFF;
PETIT_RXTX_STATE xdata   Petit_Tx_State                = PETIT_RXTX_IDLE;
unsigned char    xdata   Petit_Tx_Buf[PETITMODBUS_TRANSMIT_BUFFER_SIZE];
unsigned int     xdata   Petit_Tx_Buf_Size             = 0;

PETIT_RXTX_DATA xdata    Petit_Rx_Data;
unsigned int    xdata    Petit_Rx_CRC16                = 0xFFFF;
PETIT_RXTX_STATE xdata    Petit_Rx_State                = PETIT_RXTX_IDLE;
unsigned char    xdata   Petit_Rx_Data_Available       = FALSE;
...
void setRegisterValue(unsigned int addr, short value)
{
unsigned short i;
	for(i = 0; i < NUMBER_OF_OUTPUT_PETITREGISTERS; i++) {
		if (PetitRegisters[i].Addr == addr) {
			PetitRegisters[i].ActValue = value;
			break;
		}
	}
}

short getRegisterValue(unsigned int addr)
{
	unsigned short i;
	for(i = 0; i < NUMBER_OF_OUTPUT_PETITREGISTERS; i++) {
		if (PetitRegisters[i].Addr == addr)
			return PetitRegisters[i].ActValue;
	}
	return 0;
}

unsigned char registerExists(unsigned int addr)
{
	unsigned short i;
	for(i = 0; i < NUMBER_OF_OUTPUT_PETITREGISTERS; i++) {
		if (PetitRegisters[i].Addr == addr)
...
unsigned char registersExists(unsigned int addr, unsigned int num)
{
	unsigned short i;
	for(i = 0; i < num; i++) {
		if (registerExists(addr + i) == 0)
			return 0;
	}
	return 1;
}
...
void HandlePetitModbusReadHoldingRegisters(void)
{
    ...
    unsigned int xdata   Petit_StartAddress        = 0;
    unsigned int xdata   Petit_NumberOfRegisters   = 0;
    unsigned int xdata   Petit_i                   = 0;
    ...
    //if((Petit_StartAddress+Petit_NumberOfRegisters)>NUMBER_OF_OUTPUT_PETITREGISTERS)
		if (registersExists(Petit_StartAddress, Petit_NumberOfRegisters) == 0)
    ...
    else
    {
        ...
        for (Petit_i = 0; Petit_i < Petit_NumberOfRegisters; Petit_i++)
        {
           // unsigned short Petit_CurrentData = PetitRegisters[Petit_StartAddress+Petit_i].ActValue;
						unsigned short Petit_CurrentData = getRegisterValue(Petit_StartAddress+Petit_i);
...
void HandlePetitModbusWriteSingleRegister(void)
{
    ...
    unsigned int  xdata  Petit_Address   = 0;
    unsigned int  xdata  Petit_Value     = 0;
    unsigned char xdata  Petit_i         = 0;
   ...
   // if(Petit_Address>=NUMBER_OF_OUTPUT_PETITREGISTERS)
		if (registerExists(Petit_Address) == 0)
        HandlePetitModbusError(PETIT_ERROR_CODE_02);
    else
    {
        //PetitRegisters[Petit_Address].ActValue=Petit_Value;
				setRegisterValue(Petit_Address, Petit_Value);
...
void HandleMPetitodbusWriteMultipleRegisters(void)
{
    // Write single numerical output
    unsigned int  xdata  Petit_StartAddress            =0;
    unsigned char xdata  Petit_ByteCount               =0;
    unsigned int  xdata  Petit_NumberOfRegisters       =0;
    unsigned char xdata  Petit_i                       =0;
    unsigned int	xdata  Petit_Value                   =0;
...
{
    unsigned char xdata Result    = Petit_Rx_Data_Available;
...
{
    int xdata PetitExpectedReceiveCount=0;
...
{
    unsigned char xdata  Petit_i;
    unsigned char xdata  Petit_ReceiveBufferControl=0;
